...
import React, { Component, type ElementRef, type Element as ReactElement } from "react";
...
function isInView(el) {
    ...
    const topOffset = (getStyle(el, "padding-top") + getStyle(el, "margin-top")) * -1;
...
export default class ScrollSpy extends Component<Props, State> {
    ...
    buildNodeList = () => {
        ...
        const els = Array.from(anchorList).map(i => document.querySelector(`#${i.dataset.hash}`));
    ...
    render() {
        return <NodeResolver innerRef={this.getElements}>{this.props.children}</NodeResolver>;
    }
}
